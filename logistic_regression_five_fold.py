...
test_df = pd.read_csv(r'e:\Study\HW\ML\kaggle\ml-2024-f\cleaned_test_file.csv')
...
# Standardize numerical features (essential for Logistic Regression)
...
print("\nPerforming Stratified 5-Fold Cross-Validation with Logistic Regression...\n")
...
for train_index, val_index in skf.split(X, y):
    ...
    val_preds = lr_clf.predict(X_val_fold)
    val_proba = lr_clf.predict_proba(X_val_fold)[:, 1]
...
# Initialize Logistic Regression classifier
lr_clf_full = LogisticRegression(
    penalty='l2',
    C=1.0,
    solver='liblinear',
    random_state=42,
...
test_preds = lr_clf_full.predict(X_test)
test_proba = lr_clf_full.predict_proba(X_test)[:, 1]
...
output_path = r'e:\Study\HW\ML\kaggle\code submission\results\logistic_regression\predictions.csv' 
